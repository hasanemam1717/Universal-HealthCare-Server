...
const getMetaForDashBoard = async (user) => {
    let metadata;
    switch (user?.role) {
        case index_d_1.UserRole.SUPER_ADMIN:
            metadata = getSuperAdminMetaData();
            break;
        case index_d_1.UserRole.ADMIN:
            metadata = getAdminMetaData();
            break;
        case index_d_1.UserRole.DOCTOR:
            metadata = getDoctorMetaData(user);
            break;
        case index_d_1.UserRole.PATIENT:
...
const getDoctorMetaData = async (user) => {
    ...
    const doctorAppointmentCount = await prisma_1.default.appointment.count({
        where: {
            doctorId: doctorInfo.id
        }
    });
    const doctorPatientCount = await prisma_1.default.appointment.groupBy({
        by: ['patientId'],
        _count: {
            id: true
        }
...
const getPatientMetaData = async (user) => {
    ...
    const patientAppointmentCount = await prisma_1.default.appointment.count({
        where: {
...
const getBarChartData = async () => {
    const appointmentCountByMonth = await prisma_1.default.$queryRawUnsafe(`
